...
try {
  ...
  for (const membership of memberships) {
    await sql`
      ...
      VALUES (${membership.team_id}, ${user.id}, 'intelligence', true, ${user.id}, now())
    ...
    if (membership.role !== 'owner' && membership.role !== 'admin') {
      await sql`
        UPDATE team_members
        SET permissions = coalesce(permissions, '{}'::jsonb) || '{"intelligenceRead":true}'::jsonb
        WHERE id = ${membership.id}
      `;
    }
  }

  console.log(`Inteligencia activado para ${user.email} en ${memberships.length} equipo(s).`);
} finally {
  await sql.end();
}
