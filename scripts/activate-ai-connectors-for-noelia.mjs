...
const PLUGIN_IDS = ['grok-connector', 'claude-code-connector', 'chatgpt-connector'];
...
try {
  ...
  await sql.begin(async (tx) => {
    for (const pluginId of PLUGIN_IDS) {
      await tx`
        ...
        VALUES (${pluginId}, false, ${user.id}, now())
      ...
      for (const membership of memberships) {
        await tx`
          ...
          VALUES (${membership.team_id}, ${user.id}, ${pluginId}, true, ${user.id}, now())
  ...
  const access = await sql`
    SELECT tmp.plugin_id, u.email, tmp.team_id, tmp.enabled
    FROM team_member_plugins tmp
    INNER JOIN users u ON u.id = tmp.user_id
    WHERE tmp.plugin_id = ANY(${PLUGIN_IDS})
    ORDER BY tmp.plugin_id, lower(u.email), tmp.team_id
  `;

  const unauthorized = access.filter((row) => row.enabled && row.email.toLowerCase() !== TARGET_EMAIL);
  if (unauthorized.length) throw new Error('La verificación encontró conectores habilitados fuera de la cuenta autorizada.');

  console.log(JSON.stringify({ plugins: PLUGIN_IDS, targetEmail: TARGET_EMAIL, access }, null, 2));
} finally {
  await sql.end();
}
