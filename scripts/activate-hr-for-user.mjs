...
try {
  ...
  for (const membership of memberships) {
    await sql`
      ...
      VALUES (${membership.team_id}, ${user.id}, 'hr', true, ${user.id}, now())
    ...
    if (membership.role !== 'owner' && membership.role !== 'admin') {
      await sql`
        UPDATE team_members
        SET permissions = coalesce(permissions, '{}'::jsonb) || '{"hrRead":true,"hrWrite":true}'::jsonb
        WHERE id = ${membership.id}
      `;
    }
  }

  console.log(`RRHH activado para ${user.email} en ${memberships.length} equipo(s).`);
} finally {
  await sql.end();
}
