...
3. ~~**`scripts/seed-sales-ops-quick-actions.ts` no es idempotente.**~~ ✅ 2026-09-07: se copió `estable()` de `seed-production-skills.ts`. Verificado corriéndolo: **0 versiones nuevas, 12 sin cambios** (antes creaba 12 por corrida).
4. ~~**`ListaClientes.tsx`** rotula "ventas pendientes" un número que ahora suma ventas, asientos y suscripciones impagas.~~ ✅ 2026-09-07: dice "N pendientes de cobro" con el detalle por fuente en el tooltip.
5. **`Enfoque.tsx` de Tareas OS** debería aceptar un pedido de producción: hoy hay dos relojes de 25 minutos independientes (`Enfoque.tsx`, con `useState` que se pierde al cerrar, y `useBloqueProduccion` con `sales-ops:focus:bloque-produccion` en localStorage). **NO se hizo a propósito el 2026-09-07:** no es un arreglo sino un rediseño de una pantalla que funciona —hay que decidir si el Enfoque de Tareas pasa a sostener pedidos de producción o si son dos cosas distintas—, y esa decisión conviene tomarla mirando la pantalla.
6. **B15, decisión de producto.** La acción "Mensaje" del Focus comercial hace que la persona apruebe dos veces: aprueba el pedido y después la fila. **Decidido el 2026-09-07: queda como está**, porque un mensaje a un cliente es lo único que conviene mirar dos veces.
7. ~~**Suscripciones `cancelled` impagas** quedaron fuera de cobranzas a propósito.~~ ✅ 2026-09-07: ya no se tiran. Son un cuarto grupo, **«canceladas pendientes de proceso»** (`cancelledCount` / `cancelledByCurrency`), fuera del total porque no es plata que vaya a entrar, pero visibles porque alguien las tiene que cerrar. Hoy el equipo 2 tiene 0.
