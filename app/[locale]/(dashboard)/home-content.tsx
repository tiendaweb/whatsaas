...
import { PWAInstallSection, PWAProvider } from '@/components/pwa/PWAInstall';




function DashboardPreview({ previewHost }: { previewHost: string }) {
  const t = useTranslations('LandingPage.preview');
  const urlBar = `${previewHost}/dashboard`;
        ...
        <div className="flex h-[600px] bg-background">
          ...
          <div className="w-80 border-r border-border/40 flex flex-col bg-background/50 backdrop-blur-sm hidden md:flex">
            ...
            <div className="flex-1 overflow-hidden p-2 space-y-2">
              {[
                { name: "Demo Lead", msg: t('chat_1_msg'), time: "10:23 AM", active: true, unread: 0 },
                { name: "Sample Account", msg: t('chat_2_msg'), time: "09:45 AM", active: false, unread: 2 },
                { name: "Test Contact", msg: t('chat_3_msg'), time: "Yesterday", active: false, unread: 0 },
                { name: "Example Buyer", msg: t('chat_4_msg'), time: "Yesterday", active: false, unread: 0 },
              ].map((chat, i) => (
          ...
          <div className="flex-1 flex flex-col bg-background relative">
            <div className="h-16 border-b border-border/40 flex items-center justify-between px-6 bg-background/80 backdrop-blur-md sticky top-0 z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-sm">Demo Lead</h3>
          ...
          <div className="w-72 border-l border-border/40 bg-muted/5 hidden lg:block p-5">
             <div className="flex flex-col items-center mb-6">
                ...
                <h3 className="font-bold text-lg">Demo Lead</h3>
                <p className="text-sm text-muted-foreground">+0 (000) 000-0000</p>
             </div>
...
function SolutionsStrip({ siteName }: { siteName: string }) {
  const solutions = [
    ...
    <section className="py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="text-3xl font-bold tracking-tight md:text-4xl">{siteName} no está hecho para verse bonito. Está hecho para quitarte fricción, desorden y fugas de dinero.</h2>
...
export default async function HomePage() {
  ...
  const [plans, branding, landingContent] = await Promise.all([
    getPublishedPlans(),
    getBranding(),
    getLandingContent(tenant?.resellerId ?? null),
  ]);
  ...
  return (
    <PWAProvider>
      ...
      <section className="relative overflow-hidden pb-16 pt-16 sm:pb-24 sm:pt-20 lg:pb-32 lg:pt-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <h1 className="mx-auto mb-5 max-w-4xl text-4xl font-bold leading-[1.08] tracking-tight text-foreground animate-in fade-in slide-in-from-bottom-4 duration-700 sm:mb-6 sm:text-5xl md:text-7xl">
          ...
          <p className="mx-auto mb-8 max-w-2xl text-base leading-7 text-muted-foreground animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100 sm:mb-10 sm:text-xl sm:leading-relaxed">
            {t('hero.subtitle')}
          </p>
          
          <div className="mb-14 flex flex-col items-stretch justify-center gap-3 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200 sm:mb-20 sm:flex-row sm:items-center sm:gap-4 lg:mb-24">
            <Link href="/sign-up" className="w-full sm:w-auto">
              <Button size="lg" className="h-12 w-full rounded-full px-8 text-base shadow-lg shadow-primary/25 transition-all hover:shadow-primary/40 sm:w-auto">
                {t('hero.cta_primary')} <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </Link>
            <Link href="#features" className="w-full sm:w-auto">
              <Button variant="outline" size="lg" className="h-12 w-full rounded-full bg-background/50 px-8 text-base backdrop-blur-sm sm:w-auto">
                {t('hero.cta_secondary')}
              </Button>
            </Link>
          </div>

          <DashboardPreview previewHost={identity.hostname} />
        </div>
      </section>
      ...
      {landingContent.homeSections.map((section, index) => (
        <LandingFeatureShowcase
          ...
          section={{
            ...section,
            eyebrow: renderTenantText(section.eyebrow, identity),
            title: renderTenantText(section.title, identity),
            description: renderTenantText(section.description, identity),
            bullets: section.bullets.map((bullet) => renderTenantText(bullet, identity)),
          }}
          reverse={index % 2 === 1}
        />
      ))}

      <SolutionsStrip siteName={siteName} />
