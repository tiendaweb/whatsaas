  ...
  function handleCreatePage() {
    startCreating(async () => {
      ...
      toast.success("Página creada", {
        description: `Ahora puedes editar /${result.slug} con builder visual o código React.`,
      });
    ...
    <div className="space-y-8">
      <Card className="border-primary/20 bg-primary/5">
        <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <CardDescription>
              Cada página se edita por separado con un builder visual de 4
              secciones reutilizables. Ya no tendrás todos los contenidos
              mezclados en lote.
            </CardDescription>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Crea la página y luego entrarás a su editor individual con hero,
            métricas, highlights y CTA final.
          </CardDescription>
        </CardHeader>
        ...
        {pages.length === 0 ? (
          <Card>
            <CardContent className="p-6 text-sm text-muted-foreground">
              Aún no hay páginas adicionales. Crea la primera y te llevamos
              directo a su editor visual.
            </CardContent>
          </Card>
        ) : (
          pages.map((page) => (
            <Card key={page.id}>
              ...
              <CardContent className="space-y-4">
                <div className="rounded-2xl border border-border/60 bg-muted/30 p-4 text-sm text-muted-foreground">
                  Edita esta página individualmente con bloques reutilizables
                  del sistema: hero, métricas, highlights y CTA final.
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    ...
                    {deletingId === page.id ? "Eliminando..." : "Eliminar"}
                  </Button>
                </div>
              </CardContent>
            </Card>
