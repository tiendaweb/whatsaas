...
type FlowLikeNode = Partial<
  ReactFlowNode<AutomationCanvasNodeData, AutomationFlowNodeType>
> & {
  id?: string;
  type?: AutomationFlowNodeType;
...
function normalizeNodeData(
  ...
  switch (node.type) {
    case "start": {
      ...
      const triggerType =
        merged.triggerType ?? defaultData.triggerType ?? "first_message";
      ...
      const aiDraft = automationAIDraftMetadataSchema.safeParse(merged.aiDraft);
      ...
      return {
        ...
        keywords,
        conditions:
          typeof merged.conditions === "object" && merged.conditions !== null
            ? merged.conditions
            : (defaultData.conditions ?? {}),
        aiDraft: aiDraft.success ? aiDraft.data : undefined,
      };
    }
    case "message": {
    ...
    case "options": {
      ...
      const fallbackOptions =
        Array.isArray(defaults.options) && defaults.options.length > 0
          ? defaults.options
          : ["Option 1"];
      const options = Array.isArray(merged.options)
        ? merged.options.filter(
    ...
    case "delay": {
      const fallbackSeconds =
        typeof defaults.seconds === "number" ? defaults.seconds : 2;
      const seconds =
        Number.isInteger(merged.seconds) && Number(merged.seconds) > 0
          ? Number(merged.seconds)
          : fallbackSeconds;
      return {
        label:
    ...
    case "collect": {
      const fallbackLabel = requiredString(
        defaults.label,
        "What is your name?",
      );
    ...
    case "save_contact": {
      return {
        nameVariable:
          trimString(merged.nameVariable) ??
          (defaults.nameVariable as string | undefined),
        agentId:
          trimString(merged.agentId) ??
          (defaults.agentId as string | undefined),
        departmentId:
          trimString(merged.departmentId) ??
          (defaults.departmentId as string | undefined),
        tagId:
          trimString(merged.tagId) ?? (defaults.tagId as string | undefined),
        funnelStageId:
          trimString(merged.funnelStageId) ??
          (defaults.funnelStageId as string | undefined),
        customFields:
          typeof merged.customFields === "object" &&
    ...
    case "button_message": {
      ...
      const baseButtons = Array.isArray(merged.buttons)
        ? merged.buttons
        : Array.isArray(defaults.buttons)
          ? defaults.buttons
          : [];
    ...
    case "list_message": {
      ...
      const fallbackButtonText = requiredString(
        defaults.buttonText,
        "Open list",
      );
      const baseItems = Array.isArray(merged.items)
        ? merged.items
    ...
    case "call_to_action": {
      const fallbackBodyText = requiredString(
        defaults.bodyText,
        "Open the secure link to continue.",
      );
      const fallbackButtonText = requiredString(
        defaults.buttonText,
        "Open link",
    ...
    case "condition": {
      const baseConditions = Array.isArray(merged.conditions)
        ? merged.conditions
        : [];
      const conditions = (
...
function appendDefaultsWarningIfNeeded(
  nodeId: string,
  before: unknown,
  after: unknown,
  warnings: AutomationFlowNormalizationWarning[],
...
export function normalizeAutomationFlow(input: {
  nodes: FlowLikeNode[];
  edges: FlowLikeEdge[];
}): NormalizeAutomationFlowResult {
  ...
  const normalizedNodes = input.nodes
    .filter(
      (node): node is FlowLikeNode & { type: AutomationFlowNodeType } =>
        typeof node?.type === "string",
    )
    .map((node, index) => {
      const normalizedId = createUniqueId(
        buildStableNodeId(node, index),
        usedNodeIds,
      );
      ...
      if (normalizedNode.type === "button_message") {
        const originalButtons = Array.isArray(node.data?.buttons)
          ? node.data.buttons
          : [];
        ...
        normalizedNode.data.buttons = buttons.map((button, buttonIndex) => {
          const nextId = createUniqueId(
            requiredString(
              button.id,
              `${normalizedId}-button-${buttonIndex + 1}`,
            ),
            usedButtonIds,
          );
          if (nextId !== button.id) {
      ...
      if (normalizedNode.type === "list_message") {
        const originalItems = Array.isArray(node.data?.items)
          ? node.data.items
          : [];
        ...
        normalizedNode.data.items = items.map((item, itemIndex) => {
          const nextId = createUniqueId(
            requiredString(item.id, `${normalizedId}-item-${itemIndex + 1}`),
            usedItemIds,
          );
          const nextRowId = createUniqueId(
            requiredString(
              item.rowId,
              `${slugify(normalizedId)}-row-${itemIndex + 1}`,
            ),
            usedRowIds,
          );
      ...
      if (normalizedNode.type === "condition") {
        ...
        normalizedNode.data.conditions = (
          normalizedNode.data.conditions ?? []
        ).map((condition, conditionIndex) => {
          const nextId = createUniqueId(
            requiredString(
              condition.id,
              `${normalizedId}-condition-${conditionIndex + 1}`,
            ),
            usedConditionIds,
          );
  ...
  const createAutoEndNode = (
    sourceNode: AutomationFlowNode,
    suffix: string,
  ) => {
    const nodeId = createUniqueId(
      `${sourceNode.id}-${suffix}-end`,
      usedIdsForAutoNodes,
    );
    const node = {
      id: nodeId,
  ...
  for (const conditionNode of normalizedNodes.filter(
    ...
    const hasFallback = normalizedEdges.some(
      (edge) =>
        edge.source === conditionNode.id && edge.sourceHandle === "fallback",
    );
    if (!hasFallback) {
...
export function prepareAutomationFlowForSave(input: {
  nodes: FlowLikeNode[];
  edges: FlowLikeEdge[];
