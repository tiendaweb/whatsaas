    ...
    inputSchema: {
      ...
      properties: {
        ...
        currencies: {
          ...
          description: 'Monedas ISO en las que vende la empresa, ej. ["ARS","PYG","USD"]. El catálogo muestra un selector con estas y cada plan lleva un precio por moneda (whatspro_manage_membership_plan, campo prices). Vacío = se deducen de los planes.',
        },
        default_currency: { type: ['string', 'null'], minLength: 3, maxLength: 3, description: 'Con cuál de las anteriores abre el selector. Si no está entre currencies, se usa la primera.' },
...
const manageCompanySchema = z.object({
  ...
  currencies: z.array(z.string().trim().length(3)).max(12).optional(),
  default_currency: z.string().trim().length(3).nullable().optional(),
