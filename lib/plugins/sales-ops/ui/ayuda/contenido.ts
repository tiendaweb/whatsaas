...
export const TEMAS: Partial<Record<Vista, Tema>> = {
  ...
  focus: {
    ...
    ejemplos: [
      ...
      { situacion: 'La clasificación detectó que la etapa del embudo está mal.', queHacer: 'En la ficha, el bloque «CRM a corregir» dice qué cambiaría y lo aplica de un botón. Desde el Focus, Ejecutar ahora también corrige el CRM del contacto en pantalla con nombres del catálogo del equipo: de a un contacto, nunca en lote.' },
    ],
    reglas: [
      'Ejecutar ahora NUNCA envía un WhatsApp: redacta y deja programado. Un envío sigue pasando por proponer, aprobar y ejecutar.',
      'Ejecutar ahora corrige el CRM del contacto en pantalla (etapa, etiquetas, campos) sólo con nombres del catálogo del equipo, de a uno; nunca en lote.',
      'Registrar un cobro (desde la fila «Registrar cobro» aprobada, desde el Focus con Ejecutar ahora o desde un conector) crea la venta, el asiento y el pago en Finanzas, vincula al contacto como cliente y pasa el chat a G11.',
  ...
  dinero: {
    ...
    ejemplos: [
      { situacion: 'Dijo «te transfiero mañana» hace 5 días.', queHacer: 'Está en G9. Un mensaje amable pidiendo el comprobante; si responde con el pago, registralo desde la ficha o aprobá la fila «Registrar cobro»: crea la venta, el asiento y el pago, y el chat pasa a G11.' },
    ...
    reglas: ['Registrar un cobro (desde la fila «Registrar cobro» aprobada, desde el Focus con Ejecutar ahora o desde un conector con whatspro_sales_register_payment) crea la venta, el asiento y el pago en Finanzas, vincula al contacto como cliente y pasa el chat a G11.', 'Los clientes ya existentes no entran en lotes de envío.'],
  ...
  cola: {
    ...
    titulo: 'Cola',
    queEs: 'Todo lo que se puso a hacer, por momento: En revisión, En cola, Hechos y Descartados. Lotes, indicaciones, prompts, programados y correcciones de CRM juntos, con filtro por tipo.',
    cuando: 'Después de proponer algo y antes de que salga: acá se decide.',
    comoSeUsa: [
      'En revisión: se lee el texto completo, se corrige ahí mismo y se aprueba o se descarta. Aprobar YA lo hace: un programado queda programado, un mensaje sale, una tarea se crea, una corrección de CRM se aplica. Nada de eso pasa por la IA ni por un conector.',
      'Al conector va sólo lo que necesita leer el chat y decidir (indicaciones y prompts). Lo que tiene el dato completo se hace en el servidor al aprobar.',
      'En cola: lo aprobado que sale solo a su hora (programados) o espera a un conector. "Ejecutar" queda para lo que quedó aprobado sin salir.',
    ...
    primerUso: [
      'Abrí En revisión (o el Focus violeta): leé el primer ítem entero.',
      'Corregí lo que haga falta con Editar; después Aprobar (o Descartar). El botón dice qué va a pasar: "Aprobar y programar", "Aprobar y enviar".',
      'Lo que necesite un conector queda En cola; el resto ya está en Hechos.',
    ],
    ejemplos: [
      { situacion: 'Un lote de 20 mensajes tiene 2 con el precio mal.', queHacer: 'Abrí el lote, corregí esas 2 filas con el lápiz o quitalas con la X, y "Aprobar y enviar" el resto. Pide confirmación porque sale en el acto.' },
      { situacion: 'Un programado sin prompt espera aprobación.', queHacer: '"Aprobar y programar": queda en Programados y sale solo a la hora. No lo toca ninguna IA.' },
      { situacion: 'La clasificación dice que la etapa del CRM está mal.', queHacer: 'Aparece en En revisión como "Corregir CRM" con lo que va a cambiar: Aplicar al CRM o Descartar.' },
      ...
      { situacion: 'Un pedido espera conector y yo (o una IA desde el navegador) puedo resolverlo ya.', queHacer: 'En el Focus violeta tenés el chat con caja para contestar y los programados del contacto al lado: hacelo ahí y apretá "Lo hice a mano" con una línea de qué hiciste. Sale de la cola de conectores y queda en Hechos.' },
    ...
    reglas: [
      'Proponer no envía. Aprobar un envío SÍ envía (con confirmación). Todo lo demás que se aprueba se hace en el acto si el servidor puede.',
      'Registrar un cobro (desde la fila «Registrar cobro» aprobada, desde el Focus con Ejecutar ahora o desde un conector) crea la venta, el asiento y el pago en Finanzas, vincula al contacto como cliente y pasa el chat a G11. Aprobar la fila ES la confirmación: nunca se registra por deducción del chat.',
      'Un envío aprobado por contacto a la vez, siempre con su idempotency_key sales-ops:{actionId}.',
      'Lo que un conector propone por su cuenta espera aprobación; lo que hace dentro de un pedido ya aprobado (programar, tarea, demo, CRM) sale directo.',
    ...
    conectores: [
      ...
      'Para registrar un cobro: whatspro_sales_register_payment {chat_id, amount en unidades, currency, method, paid_on, concept, idempotency_key, confirm}, sólo desde una fila Registrar cobro aprobada o por pedido explícito de una persona.',
    ],
  ...
  audios: {
    ...
    titulo: 'Audios',
    queEs: 'La cola de Gemini tal cual la va a drenar el worker —en su orden real, con la cuota que queda hoy y la reserva para lo manual— más los bloques que la reparten en el tiempo, la prioridad por contacto y el pedido a una persona cuando no vale gastar cuota.',
    cuando: 'Cuando la cola está llena de audios que no valen la cuota, y antes de aprobar un lote a chats con algo por salir: un audio sin transcribir puede cambiar el texto.',
    comoSeUsa: [
      'En cola de Gemini: cada audio tiene su posición (#). Arriba se ve cuántos esperan, cuántos pedidos quedan hoy y cuántos se reservan para lo manual (el worker y el clasificador por cron paran ahí; Transcribir ahora y Ejecutar ahora siguen). "Sacar" lo saca de la cola y queda como quitado: el cron no lo vuelve a encolar.',
      'Bloques de trabajo: agrupan audios y deciden si se transcriben ahora, desde un día, con tope por día, o quedan en pausa. Se drenan en su orden; lo sin bloque (lo recién encolado) va antes. "Armar bloques sugeridos" los reparte por frente comercial: Dinero primero, Oportunidades y Sin clasificar con tope, Clientes y Perdidos en pausa.',
      'Orden: Primero · Adelante · Normal · Al final, por contacto o por audio. "Primero" le gana a los bloques.',
      'Pedir a Noelia (o a quien elijas arriba): los audios salen de la cola, queda una tarea asignada con los enlaces y un aviso; la ficha se escribe a mano en la pestaña Quitados y la leen el radar y los conectores.',
      'Sacar de la cola (N) en la tarjeta del contacto saca todos los suyos de una. "Nunca" además lo excluye para siempre.',
      'Sin encolar: audios que nunca entraron. "Encolar (N)" los pone con prioridad, adelante de todo.',
      'Transcribir ahora / Analizar gastan un pedido de la cuota de hoy.',
    ...
    primerUso: [
      'Mirá arriba cuántos audios hay en cola, cuánta cuota queda hoy y cuánta está reservada.',
      'Abrí Bloques de trabajo: pausá lo que no importe ahora y ponele tope a lo que sí.',
      'Recorré los primeros de la cola: sacá los que no valgan la pena (Sacar, o Sacar de la cola en el contacto).',
      'Desde Sin encolar, encolá los del cliente que te importa: entran con prioridad.',
    ],
    ejemplos: [
      ...
      { situacion: 'Un contacto manda audios personales todos los días.', queHacer: 'Nunca: sale de la lista y de la cola; lo ya transcripto queda.' },
      { situacion: 'Hay 30 audios en cola de un chat que ya se cerró.', queHacer: '"Sacar de la cola (30)" en la tarjeta del contacto. Quedan como quitados y no vuelven; desde la pestaña Quitados se pueden reponer.' },
      { situacion: 'La cola tiene 600 audios y la cuota del día alcanza para 90.', queHacer: 'Bloques: Dinero activo sin tope, Oportunidades y Sin clasificar con tope, Clientes en pausa. Se drena en ese orden y lo que no entra hoy espera a mañana.' },
      { situacion: 'A la tarde nadie puede transcribir ni ejecutar nada: el clasificador se comió la cuota.', queHacer: 'Subí la reserva para lo manual (arriba, al lado de la cuota). Lo automático para ahí; lo que pidas a mano sigue saliendo.' },
      { situacion: 'Hay 3 audios de un cliente que cierra hoy y no hay cuota.', queHacer: '"Pedir a Noelia" en la tarjeta del contacto: le llega una tarea con los audios y escribe la ficha a mano en Quitados.' },
    ],
  ...
  programados: {
    ...
    ejemplos: [
      { situacion: 'Guardé un programado con una hora que ya pasó.', queHacer: 'No se guarda: el servidor propone el próximo horario con sentido (misma hora hoy si falta, si no mañana; fuera del horario laboral, mañana a las 10) y el editor lo ofrece de un botón. Un diario o semanal se calcula en hora argentina, no en la del servidor.' },
...
export const CURSO: Leccion[] = [
  ...
  },
  {
    ...
    parrafos: [],
    puntos: [
      'No envía sin una fila aprobada. Lo que propone por su cuenta (lotes, prompts, pedidos que nacen de un programado) espera en revisión; lo que hace dentro de un pedido ya aprobado (programar, tarea, demo, CRM) sale directo.',
      'No toca el CRM en lote ni fuera del contacto que está trabajando: corrige etapa, etiquetas y campos de a un contacto, con nombres del catálogo, y sólo lo que contradice ese chat.',
      'No cobra por deducción: un cobro se registra con whatspro_sales_register_payment sólo desde una fila «Registrar cobro» aprobada o por pedido explícito de una persona (crea venta, asiento y pago, vincula al cliente y pasa el chat a G11).',
    ...
    prompts: [
      ...
      '"Registrá el cobro de este chat: 150.000 ARS por transferencia, con fecha de hoy."',
    ],
  },
  {
