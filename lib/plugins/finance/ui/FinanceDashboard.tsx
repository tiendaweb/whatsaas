...
export function FinanceDashboard() {
  ...
  const currencies = useMemo(() => Array.from(new Set(['ARS', 'USD', ...(data?.entries.map((entry) => entry.currency) ?? [])])), [data?.entries]);
    ...
    <div className="min-w-0 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      {!entries.length ? <div className="p-5"><EmptyState icon={ReceiptText} title={t('entries_empty_title')} description={t('entries_empty_description')} action={onAdd} actionLabel={t('new_entry_button')} /></div> : <div>
        ...
        {entries.map((entry) => {
          ...
          return <button key={entry.id} type="button" onClick={() => onSelect(entry)} aria-pressed={selected} className={cn('group grid w-full min-w-0 gap-3 border-b border-l-4 border-b-border border-l-transparent px-4 py-4 text-left transition last:border-b-0 hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary md:grid-cols-[minmax(0,1.5fr)_minmax(140px,1fr)_120px_112px_140px_24px] md:items-center md:px-5', selected && 'border-l-primary bg-primary/5')}>
    ...
    <div className="min-w-0 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      <div>
        {receipts.map((receipt) => {
          ...
          return <button key={receipt.id} type="button" onClick={() => setSelectedReceipt(receipt)} aria-pressed={selected} className={cn('group flex w-full min-w-0 items-center gap-3 border-b border-l-4 border-b-border border-l-transparent p-4 text-left transition last:border-b-0 hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary sm:gap-4 sm:px-5', selected && 'border-l-primary bg-primary/5')}>
