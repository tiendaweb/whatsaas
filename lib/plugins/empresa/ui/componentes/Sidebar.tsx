...
/**
 * Rail izquierdo de Empresa. Misma gramática que el del Command Center: sin
 * sombras ni gradientes, la jerarquía la hacen el peso y el espacio.
 */
  ...
  return (
    ...
    >
      <div className={cn('flex items-center gap-2.5 px-3 pt-4 pb-3', collapsed && 'justify-center px-0')}>
        ...
        {!collapsed && (
          <span className="min-w-0">
            <span className="block truncate text-sm font-semibold leading-tight">Empresa</span>
            <span className="block truncate text-[11px] leading-tight text-muted-foreground">Gestión del negocio</span>
          </span>
        )}
      </div>
      ...
      <div className="mt-auto space-y-1 border-t border-border p-2">
        {/* Atajos verticales: plegar, la vuelta al lanzador y las dos apps que
            se usan a la par de ésta. Plegado, quedan uno debajo del otro. */}
...
/**
 * Los cuatro accesos del celular, por orden de preferencia. Se toman los
 * primeros que estén disponibles: si el equipo no tiene Ventas activa, entra el
 * siguiente en vez de quedar un hueco.
 */
const PREFERIDAS_MOVIL: Vista[] = ['inicio', 'marcas', 'suscripciones', 'ventas', 'clientes', 'planes'];
    ...
    >
      ...
      <button
        ...
        className={cn(
          ...
          masActivo ? 'text-foreground' : 'text-muted-foreground',
        )}
